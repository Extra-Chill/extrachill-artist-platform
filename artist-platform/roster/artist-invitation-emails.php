...
require_once dirname( __FILE__ ) . '/../user-linking.php'; // For bp_add_artist_membership
 ...
 * @param int $artist_id The artist_profile post ID.
 * @return bool True if sent, false otherwise.
 */
function bp_send_admin_artist_membership_notification( $user_id, $artist_id ) {
    ...
    $artist_profile_url = get_permalink( $artist_id );
    $link_page_url = 'https://extrachill.link/join';

    $subject = sprintf( __( 'Welcome to your new band space on Extra Chill!', 'extrachill-artist-platform' ), $artist_name );

    $message_lines = array();
    $message_lines[] = sprintf( __( 'Hi %s,', 'extrachill-artist-platform' ), $user->display_name );
    $message_lines[] = '';
    $message_lines[] = sprintf( __( 'Your band "%s" has been migrated to the new Band Platform on Extra Chill! 🎉', 'extrachill-artist-platform' ), $artist_name );
    $message_lines[] = '';
    $message_lines[] = __( 'You now have a dedicated band profile (where your old band topic now lives) and a FREE extrachill.link page to promote your music and connect with fans.', 'extrachill-artist-platform' );
    $message_lines[] = '';
    $message_lines[] = __( 'To get started, log in at the link below. You\'ll be guided through the process of creating your free extrachill.link page:', 'extrachill-artist-platform' );
    $message_lines[] = $link_page_url;
    $message_lines[] = '';
    $message_lines[] = sprintf( __( 'You can also view and update your band profile here: %s', 'extrachill-artist-platform' ), $artist_profile_url );
    $message_lines[] = '';
    $message_lines[] = __( 'This is 100% free and gives you powerful tools to grow your audience, manage your band, and keep your fans engaged—all in one place.', 'extrachill-artist-platform' );
    $message_lines[] = '';
    $message_lines[] = __( 'You have full moderation powers in your band forum (edit, split, and manage all topics and replies—no admin panel needed).', 'extrachill-artist-platform' );
    $message_lines[] = '';
    $message_lines[] = __( 'Ready to get started? Click the links above or reply to this email if you have any questions. Welcome to the future of your band on Extra Chill!', 'extrachill-artist-platform' );
    $message_lines[] = '';
    $message_lines[] = __( '— The Extra Chill Team', 'extrachill-artist-platform' );
