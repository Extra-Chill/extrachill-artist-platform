...
function ec_acquire_artist_membership_network_lock( $lock_name ) {
	...
	// @phpstan-ignore booleanNot.alwaysFalse (defensive: retained in case lock acquisition paths change.)
	if ( ! $acquired ) {
		return false;
	}
...
function ec_update_artist_relationship_ids( $object_type, $object_id, $meta_key, $related_id, $add ) {
	...
	for ( $attempt = 0; $attempt < 5; ++$attempt ) {
		...
		if ( ! metadata_exists( $object_type, $object_id, $meta_key ) ) {
			...
			// @phpstan-ignore if.alwaysFalse (concurrency guard: another writer may have created the meta between the failed add and this check.)
			if ( metadata_exists( $object_type, $object_id, $meta_key ) ) {
				continue;
			}
			return false;
