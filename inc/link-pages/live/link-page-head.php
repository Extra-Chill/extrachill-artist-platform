...
function extrachill_artist_link_page_custom_head( $artist_id, $link_page_id ) {
	...
	echo ec_generate_css_variables_style_block( $final_vars, 'extrch-link-page-custom-vars' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- shared generator emits the final sanitized <style> block.
	...
	if ( ! empty( $font_url ) ) {
		// phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedStylesheet -- head-isomorphic critical font CSS; load-order controlled print-swap pattern.
		echo '<link rel="stylesheet" href="' . esc_url($font_url) . '" media="print" onload="this.media=\'all\'">';
		// phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedStylesheet -- noscript fallback for the critical font CSS above.
	...
	if ( ! empty( $local_fonts_css ) ) {
		echo '<style>' . $local_fonts_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- locally-hosted @font-face CSS generated by the font manager; not user input.
	}
	...
	if ( ! empty($google_tag_id) && preg_match('/^(G|AW)-[a-zA-Z0-9]+$/', $google_tag_id) ) {
		...
		// phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript -- GTM loader must be inline in <head> before any content; enqueueing would break tag timing.
