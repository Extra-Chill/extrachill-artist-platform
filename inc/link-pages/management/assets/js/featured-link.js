...
(function(manager) {
    ...
    function initAdvancedTabControls() {
        ...
        enableFeaturedLinkCheckbox.addEventListener('change', function() {
            ...
            if (isChecked) {
                ...
                // Emit event to clear featured link in preview
                document.dispatchEvent(new CustomEvent('featuredLinkCleared'));
            ...
            currentAjaxRequest.onload = function() {
                if (currentAjaxRequest.status >= 200 && currentAjaxRequest.status < 400) {
                    try {
                        ...
                        if (response.success && response.data?.og_image_url) {
    ...
    function triggerFeaturedLinkPreviewUpdate(updatedPortion = {}) {
        ...
        // Emit event to update featured link in preview
        document.dispatchEvent(new CustomEvent('featuredLinkChanged', {
            detail: { 
                featuredLink: dataForPreview
            }
        }));
