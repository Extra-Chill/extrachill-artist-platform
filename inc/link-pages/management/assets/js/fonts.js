...
(function(manager) {
    ...
    function initFontControls() {
        ...
        if (titleFontFamilySelect) {
            titleFontFamilySelect.addEventListener('change', function() {
                ...
                if (manager.customization && typeof manager.customization.updateSetting === 'function') {
                    manager.customization.updateSetting('--link-page-title-font-family', fontValue);
                }
                
                // Load Google Font if needed and emit preview event
                const fontParam = getGoogleFontParamByValue(fontValue);
                if (fontParam) {
                    loadGoogleFont(fontParam, fontValue, function(fontStack) {
                        document.dispatchEvent(new CustomEvent('titleFontFamilyChanged', {
                            detail: { fontFamily: fontValue, fontStack: fontStack }
                        }));
                    });
                } else {
                    document.dispatchEvent(new CustomEvent('titleFontFamilyChanged', {
                        detail: { fontFamily: fontValue, fontStack: getFontStackByValue(fontValue) }
                    }));
                }
            });
        }
        ...
        if (bodyFontFamilySelect) {
            bodyFontFamilySelect.addEventListener('change', function() {
                ...
                // Update CSS variable via customization system
                if (manager.customization && typeof manager.customization.updateSetting === 'function') {
                    manager.customization.updateSetting('--link-page-body-font-family', fontValue);
                }
    ...
    manager.fonts.init = initFontControls; // Expose init function
