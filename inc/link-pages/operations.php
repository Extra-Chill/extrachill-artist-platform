 ...
 *
 * A provider returns null for unsupported owners or exactly three callbacks:
 * authorize, read, and save.
 *
...
function ec_resolve_link_page_operation_target( $target ) {
	...
	if ( is_int( $target ) ) {
		...
		$allowed = array( 'link_page_id', 'owner_reference' );
		if ( array_diff( array_keys( $target ), $allowed ) ) {
...
/**
 * Invoke a callback without allowing multisite context leakage.
 *
...
/**
 * Resolve exactly one provider descriptor for a target.
 *
 ...
 * @return array{resolved:array,provider:array}|WP_Error
 */
...
/**
 * Read current Link Page data through its owner provider.
 *
 * @param int|string|array $target Operation target.
 * @return array|WP_Error
 */
