	...
	if ( null === $registry ) {
		$registry = new class() {
			...
			public function register( $name, $callback, $priority ) {
				// @phpstan-ignore function.alreadyNarrowedType,function.alreadyNarrowedType,function.alreadyNarrowedType (contract validation: public entry point, values arrive unverified at runtime.)
...
function ec_register_link_page_owner_compatibility_provider( $name, $callback, $priority = 10 ) {
	// @phpstan-ignore method.notFound (the registry helper returns a dynamically-shaped object; the method exists at runtime.)
...
function ec_parse_link_page_owner_reference( $reference ) {
	// @phpstan-ignore function.alreadyNarrowedType (contract validation: public entry point, $reference arrives unverified at runtime.)
...
function ec_format_link_page_owner_reference( $owner ) {
	// @phpstan-ignore function.alreadyNarrowedType (contract validation: public entry point, $owner arrives unverified at runtime.)
	if ( ! is_array( $owner ) ) {
...
function ec_normalize_link_page_owner_reference( $owner ) {
	// @phpstan-ignore argument.type (generic array accepted; field shape is validated inside the formatter.)
...
function ec_restore_link_page_owner_provider_context( $blog_id, $stack, $switched ) {
	...
	while ( $current_depth > $target_depth && $attempts < 100 ) {
		restore_current_blog();
		// @phpstan-ignore booleanAnd.rightAlwaysTrue (defensive: the global may hold a non-array at runtime; check retained.)
		$current_stack = isset( $GLOBALS['_wp_switched_stack'] ) && is_array( $GLOBALS['_wp_switched_stack'] ) ? $GLOBALS['_wp_switched_stack'] : array();
		$current_depth = count( $current_stack );
		++$attempts;
	}
...
function ec_collect_raw_link_page_owner_compatibility_claims( $operation, $context ) {
	// @phpstan-ignore function.alreadyNarrowedType (contract validation: public entry point, $context arrives unverified at runtime.)
...
function ec_compensate_link_page_owner_assignment( $link_page_id, $reference, $owner_meta_id, $error ) {
	...
	// @phpstan-ignore if.alwaysTrue (defensive: verifies the deletion actually took effect before reporting success.)
	...
	// @phpstan-ignore deadCode.unreachable (reachable at runtime: the defensive metadata verification above may pass.)
	return $error;
}
	...
	foreach ( $link_page_ids as $link_page_id ) {
		...
		if ( ! empty( $stored ) ) {
			...
			if ( is_wp_error( $owner ) ) {
				// @phpstan-ignore return.type,argument.type (halt helper contract spans array/WP_Error shapes; error codes are strings at runtime.)
