...
function extrachill_artist_platform_validate_link_pages_runtime() {
	...
	// @phpstan-ignore booleanOr.alwaysFalse,notIdentical.alwaysFalse,notIdentical.alwaysFalse (contract validation: the comparison must stay so an incompatible runtime is still rejected.)
	...
	if ( function_exists( 'ec_link_pages_runtime_ready' ) ) {
		...
		try {
			// @phpstan-ignore arguments.count (arity is verified via ReflectionFunction immediately above; dependency signatures may vary by runtime version.)
