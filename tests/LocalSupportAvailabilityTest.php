...
final class LocalSupportAvailabilityTest extends TestCase {
		...
		$GLOBALS['ec_test'] = array(
			...
			'blogs'               => array(
				...
				4 => array(
					...
					'post_meta'    => array(
						42 => $this->artist_meta( 142, 'charleston', array( 7, 8, 9 ) ),
						43 => $this->artist_meta( 143, 'charleston', array( 7 ) ),
						44 => $this->artist_meta( 144, 'austin', array( 7 ) ),
						45 => array_merge( $this->artist_meta( 145, 'charleston', array( 7 ) ), array( '_local_support_available' => '' ) ),
					),
					'terms'        => array(
						950 => (object) array(
							'term_id'  => 950,
	...
	private function artist_meta( int $term_id, string $scene, array $member_ids ): array {
		return array(
	...
	public function test_manager_abilities_enforce_the_exact_artist_on_permission_and_execution_paths(): void {
		...
		$this->assertFalse( $update->check_permissions( array(
			'id'        => 42,
			'available' => false,
		) ) );
	...
	public function test_private_candidate_contract_filters_and_discloses_only_valid_manager_ids(): void {
		...
		$this->assertSame( array( 'rock' ), $candidate['genres'] );
		$this->assertSame( array( 'Rock' ), $candidate['genre_labels'] );
	...
	public function test_genre_filter_intersects_resolved_slugs_instead_of_exact_matching(): void {
		...
		$this->assertSame( array( 43 ), array_column( $result['candidates'], 'artist_profile_id' ) );

		$unfiltered = extrachill_artist_platform_resolve_local_support_candidates(
			'extrachill-events-local-support',
			'charleston'
		);
		$this->assertSame( array( 42, 43 ), array_column( $unfiltered['candidates'], 'artist_profile_id' ) );
