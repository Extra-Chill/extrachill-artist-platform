...
function get_the_title( $id ) { $post = get_post( $id );
	return $post ? $post->post_title : ''; }
function get_permalink( $id ) { return 'https://artist.extrachill.com/artists/' . get_post_field( 'post_name', $id ) . '/'; }
function get_the_post_thumbnail_url( $id ) { $thumb = get_post_thumbnail_id( $id );
	return $thumb ? wp_get_attachment_url( $thumb ) : false; }
...
function get_post_status( $id ) { $post = get_post( $id );
...
function ec_render_template( $template, $args = array() ) {
	if ( 'link-section' === $template ) {
		$output = '';
		foreach ( $args['links'] ?? array() as $link ) {
			$output .= '<span class="extrch-link-page-link-text">' . esc_attr( $link['link_text'] ) . '</span>'; }
		return $output;
	}
...
function ec_combined_cache_purge_post( $post_id ) {
	...
	foreach ( is_array( $urls ) ? $urls : array() as $url ) {
		$GLOBALS['ec_test']['cache_deleted_urls'][] = $url; }
}
...
class EcCombinedSocials {
	...
	public function get( $artist_id ) { $socials = get_post_meta( $artist_id, '_artist_profile_social_links', true );
...
function extrachill_artist_platform_social_links() { static $manager;
	if ( ! $manager ) {
		$manager = new EcCombinedSocials();
	} return $manager; }
...
if ( is_wp_error( $registered ) ) {
	echo wp_json_encode( array( 'bootstrap_error' => $registered->get_error_code() ) );
	exit( 1 ); }
...
$GLOBALS['ec_test']['blogs'][4]['posts'][21]     = (object) array(
	'ID'           => 21,
	'post_type'    => 'artist_profile',
	'post_status'  => 'publish',
	'post_title'   => 'Existing Artist',
	'post_name'    => 'existing-artist',
	'post_excerpt' => '',
	'post_content' => '',
);
$GLOBALS['ec_test']['blogs'][4]['posts'][40]     = (object) array(
	'ID'          => 40,
...
$GLOBALS['ec_test']['blogs'][4]['post_meta'][40] = array(
	EC_LINK_PAGE_OWNER_META_KEY     => 'post:4:artist_profile:21',
	'_associated_artist_profile_id' => 21,
);

$created                        = ec_create_link_page( 20 );
$creation_hooks                 = array_values( array_map( static function ( $action ) { return $action[0];
}, array_filter( $GLOBALS['ec_test']['fired_actions'], static function ( $action ) { return in_array( $action[0], array( 'ec_link_page_created', 'ec_owned_link_page_created' ), true );
} ) ) );
			...
			array(
				...
				'links'         => array(
					array(
						'id'        => 'new-link',
						'link_text' => 'Song',
						'link_url'  => 'https://example.com/song',
					),
				),
			),
		),
...
$save_hooks                   = array_values( array_map( static function ( $action ) { return $action[0];
}, array_filter( $save_actions, static function ( $action ) { return in_array( $action[0], array( 'ec_link_page_save', 'ec_link_page_persistence_saved' ), true );
} ) ) );
...
if ( 'interleaving-failure' === $mode ) {
	$post_failure_save = extrachill_artist_platform_ability_save_social_links( array(
		'artist_id'    => 20,
		'social_links' => array(
			array(
				'type' => 'bandcamp',
				'url'  => 'https://bandcamp.com/authoritative',
			),
		),
	) );
}
...
if ( ! in_array( $mode, array( 'owner-save-failure', 'interleaving-failure', 'separate-contention' ), true ) ) {
	...
	$ability_results['styles']   = extrachill_artist_platform_ability_save_link_page_styles( array(
		'artist_id' => 20,
		'css_vars'  => array( '--link-page-text-color' => '#fedcba' ),
	) );
	$ability_results['settings'] = extrachill_artist_platform_ability_save_link_page_settings( array(
		'artist_id'        => 20,
		'bio'              => 'Ability bio',
		'profile_image_id' => 55,
	) );
	$ability_results['socials']  = extrachill_artist_platform_ability_save_social_links( array(
		'artist_id'    => 20,
		'social_links' => array(
			array(
				'type' => 'instagram',
...
if ( 'separate-contention' === $mode ) {
	$separate_contention['during_combined'] = ec_with_link_page_lock_scope(
		...
		static function () {
			return array(
				'social'  => extrachill_artist_platform_ability_save_social_links( array(
					'artist_id'    => 20,
					'social_links' => array(
						array(
							'type' => 'blocked',
				...
				'profile' => extrachill_artist_platform_ability_update_artist( array(
					'artist_id' => 20,
					'name'      => 'Blocked profile mutation',
				) ),
			);
		},
		'combined'
	...
	$separate_contention['social_after']    = extrachill_artist_platform_ability_save_social_links( array(
		'artist_id'    => 20,
		'social_links' => array(
			array(
				'type' => 'after',
	...
	$separate_contention['profile_after']   = extrachill_artist_platform_ability_update_artist( array(
		'artist_id' => 20,
		'name'      => 'Profile after release',
	) );
...
if ( in_array( $mode, array( 'cross-blog-1', 'cross-blog-7', 'cross-blog-failure' ), true ) ) {
	...
	$cross_result   = extrachill_artist_platform_ability_save_social_links( array(
		'artist_id'    => 20,
		'social_links' => array(
			array(
				'type' => 'cross',
	...
	$profile_result = null;
	if ( 'cross-blog-failure' !== $mode ) {
		$profile_result = extrachill_artist_platform_ability_update_artist( array(
			'artist_id' => 20,
			'name'      => 'Cross-blog profile',
		) );
	}
...
$force_hooks          = array_values( array_map( static function ( $action ) { return $action[0];
}, array_filter( $force_actions, static function ( $action ) { return in_array( $action[0], array( 'ec_link_page_created', 'ec_owned_link_page_created' ), true );
} ) ) );
...
echo wp_json_encode(
	array(
		...
		'force_new_count'             => count( get_posts( array(
			'post_type'      => EC_LINK_PAGE_POST_TYPE,
			'post_status'    => 'any',
			'fields'         => 'ids',
			'posts_per_page' => -1,
